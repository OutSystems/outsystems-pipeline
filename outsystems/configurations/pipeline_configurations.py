...
def get_conf_value(conf: str):
    ...
    # Retrieve the config value from the properties file
    ...
    # If global configuration is not yet defined at this moment then set it to its default value
    if conf not in global_config:
        global_config[conf] = eval("outsystems.vars.pipeline_vars.{}".format(conf))
